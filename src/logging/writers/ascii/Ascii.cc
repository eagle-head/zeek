...
bool Ascii::InitFormatter() {
    ...
    if ( use_json ) {
        ...
        auto string_escape_policy = threading::formatter::JSON::STRING_ESCAPE_POLICY_LEGACY;

        if ( strcmp(json_string_escape_policy.c_str(), "JSON::STRING_ESCAPE_POLICY_LEGACY") == 0 )
            string_escape_policy = threading::formatter::JSON::STRING_ESCAPE_POLICY_LEGACY;
